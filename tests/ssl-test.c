...
char* main(int argc, char **argv) {
  ...
  if (cert == NULL)
    ...
    hex = bintohex(buf, len);
    BIO_printf(outbio, "CERT-LEN: %i\n", len);
    BIO_printf(outbio, "CERT-DERHEX:\n%s\n", hex);

    //checkDANE("www.nic.cz", 443, cert, 2);
  ...
  if (chain == NULL)
    BIO_printf(outbio, "Error: Could not get a certificate chain: %s.\n", dest_url);
  else
    BIO_printf(outbio, "Retrieved the server's certificate chain: %s.\n", dest_url);  
